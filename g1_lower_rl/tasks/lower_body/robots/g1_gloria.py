...
def ppo_runner_cfg() -> RslRlOnPolicyRunnerCfg:
  return RslRlOnPolicyRunnerCfg(
    ...
    algorithm=RslRlPpoAlgorithmCfg(
      ...
      #
      # **只在"从成熟策略续训"上验证过，从零训练未测。** 早期那一段不用担心：实测
      # σ 在前 120 迭代由 surrogate 主导地从 1.00 崩到 0.60（-6.3e-3/iter），而本次
      # 改动带来的漂移变化只有 -3.2e-4/iter，占 5%。要留意的是 200~800 迭代那段
      # σ 的**反弹**（0.60 -> 0.84），它由熵项推动，而那正好是学会走路的阶段
      # （reward 3.8 -> 29.9）。0.002 会压制这个反弹，若从零训练卡在"会站不会走"，
      # 先把这一项调回 0.01，等能走了再降下来。
      entropy_coef=0.002,
