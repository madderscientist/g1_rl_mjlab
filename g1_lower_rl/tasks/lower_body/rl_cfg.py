...
def ppo_runner_cfg_gru() -> RslRlOnPolicyRunnerCfg:
  ...
  # 所以做成课程：先用 0.01 把走路学出来，再逐档降到 0.002。
  #
  # **第一档必须赶在 σ 跨过 1.0 之前。** 曾经把它放在 4000，结果根本没机会生效：
  # 实测 σ 0.87@1000 单调涨到 1.02@2900，逐检查点确定崩溃就卡在这条线上——
  # 2900（σ=1.02）站立正常、3000 尚可、3100（σ=1.03）两个站立场景全 100% 摔。
  #
  ...
  cfg.algorithm.entropy_stages = (
    (0, 0.01, math.inf),
    (2500, 0.004, 0.55),  # σ≈0.99 时先拉一档，离悬崖还有余量
    (3000, 0.002, 0.35),  # σ* 实测 0.30~0.32
  )
