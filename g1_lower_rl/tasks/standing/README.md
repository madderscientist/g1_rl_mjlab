...
站成什么样是优化的**结果**，不是输入。这份配置里没有任何一项告诉机器人「腰要正」
「重心要在中间」「站到 0.72 m」——那些全部删掉了，理由见第 3 节。

```bash
# GPU 0（列表字面量必须加引号）
PYTHONPATH=. MUJOCO_GL=egl python scripts/train.py G1-Gloria-Stand \
    --env.scene.num-envs=4096 --mirror-schedule '()' --gpu-ids '[0]' \
    --agent.max-iterations 12001 --agent.run-name stand_v4
...
上肢在实机上由 VR IK 自行驱动，它摆到哪儿直接决定质心落在哪儿；把手臂挡在观测外面，
下肢就只能从骨盆姿态里事后反推，等于把一个可测的量硬做成扰动。事件里的
`reset_arm_pose` / `arm_pose_drift` 会把手臂在整个可达范围内摆起来，所以这 14 维
在训练分布里是有方差的。

**两个夹爪偏心轴不进观测。** 它们在训练里恒为 0，观测归一化学到的标准差接近零，
真机上夹爪一开合就会被除成巨值盖掉平衡信号——实测夹爪偏 0.2 rad 造成的动作扰动是
左膝偏同样角度的 5 倍。

延时为什么不是每拍重采：滞后量每步换一次等于给关节速度叠了一层白噪声。
`delay_hold_prob=0.98` 让它在一局内几乎恒定，这是机器人的属性，不是每一拍的属性。
...
只有 9 项。**目标函数只有 `power` 一项，其余 8 项全是「别摔、别趴下」的约束。**

| 项 | 权重 | 定义 | 角色 |
|---|---:|---|---|
| `power` | **-20.0** | $\sum_{j\in\text{下肢15轴}} (\tau_j/\tau^{max}_j)^2$ | **目标** |
| `feet_grounded` | +1.0 | 双脚同时着地时为 1，否则 0 | 约束（唯一正向项） |
...
**所以是平方，不是 L1。** 功率本来就正比于力矩平方，用绝对值反而不对。

**所以按额定归一化。** 同族执行器的 $R$ 与峰值电流接近，上式那个系数
$\propto 1/\tau_{max}^2$。不归一化的话 ankle（额定 50）出 1 N·m 和 knee（额定 139）
出 1 N·m 记同样的账，而两者的发热完全不是一回事。

**所以是求和，不是均值。** 总功率是各关节相加。上一版取了均值，等于把每笔账除以 15。

**为什么手臂不计入。** 实测默认站姿下手臂 14 轴的 $\sum(\tau/\tau_{max})^2$ 是
0.09–2.5，下肢 15 轴只有 0.008–0.13——手臂占 90% 以上，且主要由事件采到的手臂位姿
决定，策略控制不了。算进去只会把可控信号淹在外生方差里。**手臂仍然在观测里，只是
不计费**：位姿要看得见（决定质心），电耗不归它管（上层 VR IK 的事）。

**权重定标（实测，默认站姿静置 600 步）：** 下肢的和在直腿时 0.008、膝屈 0.6 rad 时
0.020，差值约 0.012。乘 -20 后单拍 0.24、整局 240，与终止罚 -200 同量级——足以决定
姿势。零动作静置时 `power` 单拍 -0.0068，是唯一正向项 `feet_grounded`（+0.0024）的
2.9 倍，即这一项确实是主导。

### 3.2 `height_floor` —— 为什么是下限而不是打靶

**这是上一版烧电机的直接原因。** 那一版写的是
$2.0\cdot\exp[-((h-0.72)/0.06)^2]$，而实测自然站姿高度是 **0.78**。站直就偏目标
6 cm = 一个 $\sigma$，奖励从 1.0 掉到 0.37，单拍亏 1.26；而当时整个省力项的量级才
0.07。于是策略的最优解就是**蹲到 0.72 再硬扛力矩**——实机上看到的「腿都是弯的」
和腰部电机过热，就是这个奖励在如实执行。
...
取 L1 而不是平方：平方在刚跌破下限处梯度为零，拦不住缓慢下沉。
-40 时低 1 cm 罚 0.4/拍，和唯一那个正向项同量级。

0.68 的取法：自然站姿 0.78，留 10 cm 给抗扰动的屈膝缓冲；再低就不是「站」了。
...
**删 `joint_limits` 的代价要知道：** 策略可能学会把关节顶到**硬**限位上让机械限位
承重（那确实是零力矩，也确实是字面意义上的最省电）。上机前看一眼实际关节角，如果
发现膝长期贴在行程端点，那不是训练出错，是它找到了最优解——但机械上不一定接受。

### 3.4 为什么正向项只留一个

`feet_grounded` (+1.0) 是唯一的正向项，留着它是为了让「活着」本身有收益：全是罚项
时，提前摔倒反而是止损。算一下账——站立每拍净收益约
$-0.0068 + 0.0024 = -0.0044$；而立刻摔倒是 -200 一次性 + 之后什么都拿不到，
按 50 步一局折算是 -4/拍。站着比躺下好三个数量级，退化解不成立。
...
（本任务没有步态时钟）。这么做是有教训的：第一版自己配（外力 ±60 N 同时压在两个
夹爪和躯干、每 1.5–3.5 s 换向）实测只能撑 3.7 s；换成官方那一小套之后回合长度回到
988 步，但又漏掉了手臂力矩冲量、手臂位姿漂移、夹爪负载、左右独立的地面摩擦。
...
对应 0.717，所以 `max_tilt=0.25`（约 14°）留足余量；至少 60% 的环境达标才放下一档。
...
观测里有 0–2 拍延时，严格来说不是马尔可夫的，循环结构理论上能把滞后补回来；
行走任务上 GRU 赢得很明显，但站立是近似静态的镇定问题、观测已含全身关节角，
优势未必迁移。要做对照的话照抄 `lower_body/rl_cfg.py` 里的
`ppo_runner_cfg_gru()` 注册一个 `G1-Gloria-Stand-GRU` 即可。
