...
PYTHONPATH=. MUJOCO_GL=egl python scripts/train.py G1-Gloria-Stand \
    ...
    --agent.max-iterations 5000 --agent.run-name stand

# 开左右镜像增强（DUP）。从第 0 迭代就开：曾经把它推迟到 1000 迭代，实测在**它还没介入的
# 那段窗口里**（迭代 400/800/1000）回合长度反而是 73/121/142，低于全程 0.5 的 87/142/164，
# 所以「早期介入拖慢训练」这个猜想不成立。两者最终都卡在 ~200/1000，瓶颈是 power 权重
# 而不是 DUP，见 3.1。
PYTHONPATH=. MUJOCO_GL=egl python scripts/train.py G1-Gloria-Stand \
    --env.scene.num-envs=4096 --gpu-ids '[0]' \
    --mirror-schedule '((0,0.5),)' \
    --agent.max-iterations 12001 --agent.run-name stand_v7_dup
...
**权重定标（实测，4096 环境训练分布）：** 训练中 power ≈ 0.128，非 `power` 项加起来的
逐拍净收益是 +0.0692。原来的 -20 吃掉其中 74%，站满一局（1000 拍）只值 +13.9，而摔
一次仅罚 -3.98——两次重训的回合长度因此都卡在 ~200/1000 走平。**-5 时它占 18.5%，
站满一局值 +52.4**，相对终止代价有 13 倍余量。

下限由退化解定：顶膝限位那条解 power = 1.77（GPU 复跑实测），-5 时单拍要付 0.177，
是全部逐拍净收益的 3.1 倍，仍然被封死。上限由「好姿态应当近乎免费」定：静止直腿
 power = 0.011，-5 时单拍只付 0.0011。

**不能照抄旧值。** -20 是按修复前的读数标的，那时 `power` 实际量的是手臂
（`actuator_force` 被用 `joint_ids` 索引，见 commit e0f8b48），下肢真实量级大一个数量级。
...
**不能取 0.25。** 本任务没有任何一项要求骨盆摆正（见 3.3），而 `reset_arm_pose` 会把手臂摆到
全可达范围、质心随之偏移，策略就歪着站来代偿。实测 512 环境稳态 tilt 分布
 p10=0.174 / p50=0.292 / p90=0.432，取 0.25 只有 33% 达标，闸门永远开不了。实际后果是
`stand_v7` 跑了 5800 迭代、整套扰动全程停在第 0 档（全关），回合长度 983/1000 是在零扰动
平地上取得的。

**已知缺陷：这个判据有反向激励。** 它量的是全体环境的当前倾角，而复位瞬间骨盆是正的
（实测达标率 100%）。所以摔得越勤、刚复位的环境占比越高，分数反而越高——回合长度 200
的 `stand_v6` 分数 0.64，而长度 983 的 `stand_v7` 只有 0.33。放宽阈值只是绕开，根治要改成
按存活率度量。
